...
# 全局初始化 - 统一创建所有必要目录（避免云环境路径错误）
os.makedirs("uploads/resumes", exist_ok=True)
os.makedirs("uploads/audio", exist_ok=True)
os.makedirs("knowledge", exist_ok=True)

# ========== 只修复错误的CSS（保留所有原有样式，删除冲突部分） ==========
st.html("""
<style>
/* 全局背景 */
...
# 已注释：面试录音分析功能（依赖whisper，云环境部署报错）
# elif menu == "🎙️ 面试录音分析":
    ...
    if submit_button and user_in:
        ...
        with chat_container:
            with st.chat_message("assistant"):
                ...
                full_response = ""
                try:
                    # 使用agents.py中统一的LLM获取方式（和其他功能保持一致）
                    llm = agents.get_llm()
