...
# 全局变量，避免重复加载模型（优化性能）
# WHISPER_MODEL = None  # 【注释】暂时禁用
# SUPPORTED_AUDIO_FORMATS = {"mp3", "wav", "m4a", "flac", "ogg"}  # 【注释】暂时禁用
...
#         # 使用 Streamlit 缓存资源，避免重复加载（关键优化）
#         @st.cache_resource(show_spinner=False)
#         def _load_whisper_model(_model_name: str, _device: str):
#             """内部缓存函数，仅加载一次模型"""
#             # 分阶段加载，打破长时间阻塞，让Streamlit同步前端状态
#             time.sleep(0.1)  # 给前端100ms同步时间
#             model = whisper.load_model(
#                 name=_model_name,
#                 device=_device,
#                 # 使用系统临时目录，适配 Streamlit Cloud 存储限制
